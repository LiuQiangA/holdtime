...
export default {
  name: "login",
  mounted: function() {
    //判断有无推送消息，无则正常登录流程，否则走推送消息流程
    if (appDB.get("messageParams")==null) {
      alert("messageParams为空")
      ...
      if (appDB.get("companyInfo") == null) {
        ...
        alert("进入带判断类型")
        $("#loginCont").hide();
        var type = appDB.get("messageParams").type;
        if(type=="APPROVE"){
            this.$router.push("/shenpiList");
        } else if(type=="MESSAGE_DETAIL"){
  ...
  methods: {
    //给APP传递手机号
    tellphone() {
      window.xiaolu.passMobile(this.phoneNumber);
    },
    //获取验证码
    getNum() {
      if (this.phoneNumber == "") {
      ...
      this.$http
        .get(
          "http://123.56.228.104:9091/api/user/getVerificationCode/" +
            this.phoneNumber
        )
        .then(response => {
          var d = response.data;
    ...
    Active() {
      ...
      var _this = this;
      this.$http
        .post("http://123.56.228.104:9091/api/user/activate", {
